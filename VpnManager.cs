...
namespace NetworkTroubleshooter
{
    public class VpnManager
    {
        ...
        /// <summary>
        /// 获取当前活动 VPN 连接的状态信息（用于界面显示）
        /// </summary>
        /// <returns>格式化的 VPN 状态文本</returns>
        public string GetActiveVpnInfo()
        {
            try
            {
                ...
                {
                    return "VPN 状态：无活动 VPN 连接";
                }

                var sb = new StringBuilder();
                sb.AppendLine("VPN 状态：已连接");
                foreach (var conn in activeConnections)
                {
                    sb.AppendLine($"  • {conn.EntryName}");
                }
                return sb.ToString().TrimEnd();
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"GetActiveVpnInfo error: {ex.Message}");
                return "VPN 状态：无法检测（请检查权限）";
            }
        }
    }
}
